...
public class Exercise7Test extends CompanyDomainForKata
{
    ...
    public void sortedTotalOrderValue()
    {
        MutableList<Double> sortedTotalValues = this.company.getCustomers()
        		.collect(Customer.TO_TOTAL_ORDER_VALUE).toSortedList();
    ...
    public void maximumTotalOrderValue()
    {
        Double maximumTotalOrderValue = this.company.getCustomers()
        		.collect(Customer.TO_TOTAL_ORDER_VALUE).max();
    ...
    {
        String tildeSeparatedNames = ArrayIterate.collect(this.company.getSuppliers(), Supplier.TO_NAME)
        		.makeString("~");
        Assert.assertEquals(
    ...
    public void deliverOrdersToLondon()
    {
    	this.company.getCustomers().select(Predicates.attributeEqual(Customer.TO_CITY, "London"))
    			.flatCollect(Customer::getOrders).forEach(Order::deliver);
